...
public class UpdateRepeaterProtocolValidatorTests
{
    private readonly UpdateRepeaterProtocolValidator _sut = new();
    ...
    public void Validate_WhenNumericFieldIsNegative_ShouldHaveError(string fieldName)
    {
        ...
        {
            nameof(UpdateRepeaterProtocolCommand.RestSeconds) => CreateValidCommand() with { RestSeconds = -1 },
            nameof(UpdateRepeaterProtocolCommand.HandSwitchSeconds) => CreateValidCommand() with { HandSwitchSeconds = -1 },
            nameof(UpdateRepeaterProtocolCommand.SetRestSeconds) => CreateValidCommand() with { SetRestSeconds = -1 },
            nameof(UpdateRepeaterProtocolCommand.CountdownSeconds) => CreateValidCommand() with { CountdownSeconds = -1 },
    ...
    private static UpdateRepeaterProtocolCommand CreateValidCommand()
    {
        return new UpdateRepeaterProtocolCommand(
            Id: Guid.NewGuid(),
            Name: "Protocol A",
            WeightPercentage: 80,
            RepsPerSet: 6,
