...
public class CreateRepeaterProtocolValidatorTests
{
    private readonly CreateRepeaterProtocolValidator _sut = new();
    ...
    public void Validate_WhenNumericFieldIsNegative_ShouldHaveError(string fieldName)
    {
        ...
        {
            nameof(CreateRepeaterProtocolCommand.RestSeconds) => CreateValidCommand() with { RestSeconds = -1 },
            nameof(CreateRepeaterProtocolCommand.HandSwitchSeconds) => CreateValidCommand() with { HandSwitchSeconds = -1 },
            nameof(CreateRepeaterProtocolCommand.SetRestSeconds) => CreateValidCommand() with { SetRestSeconds = -1 },
            nameof(CreateRepeaterProtocolCommand.CountdownSeconds) => CreateValidCommand() with { CountdownSeconds = -1 },
    ...
    private static CreateRepeaterProtocolCommand CreateValidCommand()
    {
        return new CreateRepeaterProtocolCommand(
            Name: "Protocol A",
            WeightPercentage: 80,
            RepsPerSet: 6,
            NumberOfSets: 1,
